...
const SOURCE = [{ '@id': 'http://localhost:8080/Plone/another-case-study' }];

function renderEdit(
  props: Partial<React.ComponentProps<typeof CaseStudyMetadataEdit>> = {},
) {
  return render(
    <CaseStudyMetadataEdit
...
describe('CaseStudyMetadataEdit', () => {
  it('names the block in the editor placeholder', () => {
    const { getByText } = renderEdit();
    expect(getByText('Case Study Metadata Block')).toBeTruthy();
  });
  ...
  it('shows the picked source path', () => {
    const { getByText, container } = renderEdit({
      data: { '@type': 'case_study_metadata', case_study_source: SOURCE },
    });
  ...
  it('treats an empty source list as no selection', () => {
    const { getByText } = renderEdit({
      data: { '@type': 'case_study_metadata', case_study_source: [] },
    });
    expect(getByText('Current context page properties')).toBeTruthy();
  });
